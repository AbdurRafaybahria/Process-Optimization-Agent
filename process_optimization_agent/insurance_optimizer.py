...
def detect_user_involvement(process: Process) -> bool:
    ...
    # Decision logic:
    # Strong admin indicators mean it's back-office work
    if admin_score >= 5:
        return False  # Administrative only - use manufacturing visualization
    
    # If admin score is significantly higher than user score, it's admin-only
    if admin_score > user_score * 1.5 and user_score < 5:
        return False  # Administrative only - use manufacturing visualization
    
    # If user score is high and significantly higher than admin, user is involved
    if user_score >= 5 and user_score > admin_score:
        return True  # User-facing - use healthcare visualization
    
    # Default: if unclear, assume administrative (most insurance back-office is admin-only)
    return False
...
class InsuranceScenarioDetector:
    ...
    def _check_standard_billing(self, process_text: str, task_names: List[str]) -> Tuple[float, List[str]]:
        """Check for standard billing scenario"""
        score = 0.0
        reasons = []
        
        # Standard sequence: Bill → Verify → Submit → Reconcile
        sequence_terms = ['bill', 'verif', 'submit', 'reconcil']
        matches = sum(1 for term in sequence_terms if any(term in task for task in task_names))
        
        if matches >= 3:
            score += 10
            reasons.append(f"Standard billing sequence detected ({matches}/4 steps)")
        
        if 'medical billing' in process_text or 'billing and insurance' in process_text:
            score += 5
            reasons.append("Medical billing process name detected")
...
class InsuranceProcessOptimizer:
    ...
    def _optimize_by_scenario(self, process: Process, scenario_type: InsuranceScenarioType) -> Tuple[Schedule, InsuranceMetrics]:
        ...
        if scenario_type == InsuranceScenarioType.STANDARD_BILLING:
            ...
            return self._optimize_pre_authorization(process)
        elif scenario_type == InsuranceScenarioType.EMERGENCY_CARE:
            return self._optimize_emergency_care(process)
        elif scenario_type == InsuranceScenarioType.DENIED_APPEALS:
            return self._optimize_denied_appeals(process)

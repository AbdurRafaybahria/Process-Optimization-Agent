...
from process_optimization_agent import CMSDataTransformer, IntelligentOptimizer, ProcessIntelligence, ProcessType
from process_optimization_agent import Process, Task, Resource, UserInvolvement, DependencyDetector
from process_optimization_agent.Optimization.models import Skill, SkillLevel

# Banking-specific imports
try:
    from process_optimization_agent.scenarios.banking.banking_detector import BankingProcessDetector
    from process_optimization_agent.scenarios.banking.banking_optimizer import BankingProcessOptimizer
    from process_optimization_agent.scenarios.banking.banking_metrics import (
...
def test_process_detection(json_file):
    ...
    try:
            ...
            if result.admin_metrics:
                ...
                if classification.process_type == ProcessType.MANUFACTURING:
                    ...
                    try:
                        ...
                        from process_optimization_agent.scenarios.banking.banking_metrics import MultiObjectiveOptimizer
                        objectives = [
        ...
        if result.schedule:
            ...
            try:
                from process_optimization_agent.visualization import Visualizer
                ...
                if classification.process_type.value == 'insurance':
                    ...
                    if insurance_result and hasattr(insurance_result, 'user_involved'):
                        ...
                        before_metrics = {
                            'duration': insurance_result.optimized_metrics.before_process_time / 60,  # Convert minutes to hours
                            'cost': insurance_result.optimized_metrics.before_cost,
